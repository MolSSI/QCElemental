...
#       me                        'electron mass'                             = 9.10938215E-31       # Electron rest mass (in kg)
    ...
    def write_psi4_header(self, filename='physconst.h'):
        ...
        text = []
        text.append('#ifndef _psi_include_physconst_h_')
        text.append('#define _psi_include_physconst_h_')
        text.append('')

        text.append('/*')
        text.append(' * This file is autogenerated from qcelemental python module')
        text.append(' * Make sure that you comment any new additions to this, so they are inlined into the manual')
        text.append(' * as well as the phyconst.py python file.  Note the format of the existing comment markers.')
        text.append(' */')
        text.append('')

        text.append('// clang-format off')
        text.append('#define pc_pi {}'.format(pi))
        text.append('#define pc_twopi {}'.format(tau))
