...
def to_string(
    ...
    units: str | None = None,
    *,
    atom_format: str | None = None,
    ghost_format: str | None = None,
    width: int = 17,
    prec: int = 12,
    ...
    dtype
        {"xyz", "cfour", "jaguar", "nwchem", "molpro", "orca", "turbomole", "qchem"}
    ...
    if dtype in ["xyz", "xyz+"]:
        ...
        if units.capitalize() != "Angstrom":
            raise ValueError("Jaguar Cartesian coordinates must be in Angstroms.")
        ...
        for atom_index, (symbol, real, xyz) in enumerate(zip(molrec["elem"], molrec["real"], geom), start=1):
            ...
            atom = [f"{label:<{width}}"]
            atom.extend(f"{coordinate:>{width}.{prec}f}" for coordinate in xyz)
            atoms.append("  ".join(atom))

        smol = ["&zmat"] + atoms + ["&"]

        data.fields.extend(["molecular_charge", "molecular_multiplicity", "real", "units"])
        data.keywords = {
            "molchg": int(molrec["molecular_charge"]),
            "multip": molrec["molecular_multiplicity"],
        }
