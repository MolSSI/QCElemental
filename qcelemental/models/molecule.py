...
class Molecule(ProtoModel):
    ...
    symbols: Array[str] = Field(  # type: ignore
        ...,
        description="An ordered (nat,) array-like object of atomic elemental symbols in title case. The index of "
        "this attribute sets atomic order for all other per-atom fields like ``real`` and the first "
        "dimension of ``geometry``. Ghost/Virtual atoms must have an entry here in ``symbols``; ghostedness is "
    ...
    fragments_: Optional[List[Array[np.int32]]] = Field(  # type: ignore
        None,
        description="List of indices grouping atoms (0-indexed) into molecular fragments within the Molecule. This is a list of shape "
        ...
        "per-atom settings like ``symbols`` and ``real``. "
        "Fragment ordering is fixed; that is, a consumer who shuffles fragments must not reattach the input (pre-shuffling) molecule schema instance to any output (post-shuffling) per-fragment results (e.g., n-body energy arrays).",
        shape=["nfr", "<varies>"],
    )
